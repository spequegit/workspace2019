...
public class LedPwmController {
	...
	public void dimCheck() throws InterruptedException, IOException {
		...
		// GpioPinDigitalOutput dirPin_C =
		// gpio.provisionDigitalOutputPin(dir_C);
		// GpioPinDigitalOutput dirPin_D =
		// gpio.provisionDigitalOutputPin(dir_D);
		boolean dirA = false;
		boolean dirB = false;
		boolean dirC = false;
		boolean dirD = false;
		...
		while (true) {
			...
			// SoftPwm.softPwmWrite(PWM_PIN_C, Math.abs(speed - (dirC ? 100 :
			// 0)));
			// SoftPwm.softPwmWrite(PWM_PIN_D, Math.abs(speed - (dirD ? 100 :
			// 0)));
		}
	}
